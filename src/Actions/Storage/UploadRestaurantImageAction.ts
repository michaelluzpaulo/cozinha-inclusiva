...
export class UploadRestaurantImageAction {
  static async execute(
    ...
    const fileName = customFileName
      ? `restaurants/${customFileName}.${fileExt}`
      : restaurantId
      ? `restaurants/restaurant_${restaurantId}_${Date.now()}.${fileExt}`
      : `restaurants/${Date.now()}-${Math.random()
          .toString(36)
          .substring(2)}.${fileExt}`;
